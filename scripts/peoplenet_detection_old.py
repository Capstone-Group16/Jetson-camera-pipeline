...
def pgie_src_pad_buffer_probe(pad, info, u_data):
    """Extract detection metadata from each frame, reads detection results from nvinfer per frame in buffer"""
    gst_buffer = info.get_buffer()
    if not gst_buffer:
        return Gst.PadProbeReturn.OK

    batch_meta = pyds.gst_buffer_get_nvds_batch_meta(hash(gst_buffer)) #fetch buffer's metadata, hash is python's pointer way of pointing to a C object (nvdsbatchmeta *batch_meta = gst_buffer_get_nvds_batch_meta(buffer))
    l_frame = batch_meta.frame_meta_list

    while l_frame is not None:
        try:
            frame_meta = pyds.NvDsFrameMeta.cast(l_frame.data) ##contains all frame metadata
        except StopIteration:
            break

        #https://docs.nvidia.com/metropolis/deepstream/6.1.1/dev-guide/python-api/PYTHON_API/NvDsMeta/NvDsFrameMeta.html
        frame_number = frame_meta.frame_num
        num_rects = frame_meta.num_obj_meta
        # Get buffer presentation timestamp (PTS)
        buf_pts = frame_meta.buf_pts #get current nanosecond count from gst_buffer in nvdsbatchmeta
        timestamp_str = format_timestamp(buf_pts)

        # Count objects by class and collect bounding boxes
        obj_counter = {
            PGIE_CLASS_ID_PERSON: 0,
            PGIE_CLASS_ID_BAG: 0,
        ...
        # Print detection results with timestamp
        if not silent:
            print(f"[{timestamp_str}] Frame {frame_number:05d} | "
                  f"Objects: {num_rects:3d} | "
                  f"person: {obj_counter[PGIE_CLASS_ID_PERSON]:2d}, "
                  f"bag: {obj_counter[PGIE_CLASS_ID_BAG]:2d}, "
                  f"face: {obj_counter[PGIE_CLASS_ID_FACE]:2d}")
            # Print bounding boxes for each detection
            for det in detections:
                print(f"    {det['label']:6s} conf={det['confidence']:.2f} "
                      f"bbox=({det['x']:4d}, {det['y']:4d}, {det['w']:4d}, {det['h']:4d})") #grid position, xy, width and height of box

        # Update FPS tracker
        global fps_tracker
        if fps_tracker:
            fps_tracker.update(frame_meta.pad_index)
...
def main(input_sources, config_file=None, tcp_port=None):
    ...
    for elem in [queue1, queue2, queue3, queue4, queue5, pgie, tiler, nvvidconv, nvosd, sink]:
        pipeline.add(elem)
